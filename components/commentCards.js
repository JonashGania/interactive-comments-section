...
const replyCard_1 = __importDefault(require("./replyCard"));
...
const DisplayComments = (comments) => {
    ...
    const sortedComments = (0, utils_1.sortCommentsByScore)(comments);
        ...
        <div class="user-comment-container">
            ...
            <div class="comment-content-container">
                <div class="content-head">
                    ${comment.user.username === data_1.data.currentUser.username
            ? `
                        <div class="comment-head-wrapper">
                            ...
                            <span class="comment-date-created">${(0, utils_1.getDateCreated)(comment.createdAt)}</span>
                        </div>
    ...
    `;
        ...
        const replyLists = (0, replyCard_1.default)(comment.replies, comment.id, comment.replies.length);
        const userAvatarContainer = (commentElement.querySelector(".user-avatar-container"));
        const userReplyContainer = (commentElement.querySelector(".user-reply-container"));
        const form = commentElement.querySelector(".reply-form");
        const replyButton = (commentElement.querySelector(".reply-button"));
        const addReply = (commentElement.querySelector(".add-reply"));
        ...
        const updateBtn = (commentElement.querySelector(".update-button"));
        const updateContentContainer = (commentElement.querySelector(".update-content-container"));
        const commentContent = (commentElement.querySelector(".user-comment-content"));
        const upvoteBtn = (commentElement.querySelector(".upvote-btn"));
        const downvoteBtn = (commentElement.querySelector(".downvote-btn"));
        const voteScoreSpan = (commentElement.querySelector(".vote-score"));
        form.addEventListener("submit", (event) => {
            event.preventDefault();
            (0, eventHandlers_1.replySubmission)(addReply, comment.id, comment.replies.length, comment.user.username);
            form.classList.add("close");
        });
        if (cancelBtn && deleteBtn && yesDeleteBtn && overlay && modalContainer) {
            (0, eventHandlers_1.handleDeleteComment)(cancelBtn, deleteBtn, yesDeleteBtn, overlay, modalContainer, comment.id);
        }
        if (editBtn) {
            (0, eventHandlers_1.handleUpdatingComment)(editBtn, updateBtn, updateContentContainer, commentContent, commentElement, comment.id);
        }
        if (replyButton) {
            (0, eventHandlers_1.handleReplyButton)(replyButton, form, addReply);
        }
        (0, eventHandlers_1.handleUpdateCommentScore)(upvoteBtn, downvoteBtn, voteScoreSpan, comment.id);
        userAvatarContainer.appendChild(userAvatar);
        userReplyContainer.appendChild(replyLists);
        commentsList.appendChild(commentElement);
    });
};
exports.default = DisplayComments;
